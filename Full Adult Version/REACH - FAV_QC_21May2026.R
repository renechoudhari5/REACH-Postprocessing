...
this_id <- current_export_clean %>% filter(secret_user_id == "9999-999") # focusing on one subject
View(this_id)
...
# 2. Standardize assessment names across participants -- update as needed
for_comp <- for_comp %>%
  mutate(activity_flow_name = recode(activity_flow_name,
                                     "Evening Assessment (Female)" = "Evening Assessment"))
