...
random <- get_random(current_export_clean, secret_user_id, date, activity_name)
...
schedule_history_long <- schedule_history %>%
  ...
  unnest(date) %>%
  crossing(activity_name = assessments) %>%
  ungroup() %>%
  select(c(secret_user_id, date, activity_name))
...
rows_to_add <- schedule_history_long %>%
  anti_join(for_comp, by = c("secret_user_id", "activity_name", "date")) %>%
...
# 1. Identify completed entries (response to since_pain, which is last question)
for_comp$completed = ifelse(is.na(for_comp$since_pain),0,1)
...
subj_x_type = for_comp %>%
  group_by(secret_user_id, activity_name) %>% 
